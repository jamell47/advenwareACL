"use strict";
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
const express_1 = require("express");
const auth_routes_1 = __importDefault(require("./auth.routes"));
const student_routes_1 = __importDefault(require("./student.routes"));
const application_routes_1 = __importDefault(require("./application.routes"));
const document_routes_1 = __importDefault(require("./document.routes"));
const placement_routes_1 = __importDefault(require("./placement.routes"));
const payment_routes_1 = __importDefault(require("./payment.routes"));
const notification_routes_1 = __importDefault(require("./notification.routes"));
const messaging_routes_1 = __importDefault(require("./messaging.routes"));
const support_routes_1 = __importDefault(require("./support.routes"));
const admin_routes_1 = __importDefault(require("./admin.routes"));
const agent_routes_1 = __importDefault(require("./agent.routes"));
const organization_routes_1 = __importDefault(require("./organization.routes"));
const organisation_routes_1 = __importDefault(require("./organisation.routes"));
const organisationRequest_routes_1 = __importDefault(require("./organisationRequest.routes"));
const organisationAdmin_routes_1 = __importDefault(require("./organisationAdmin.routes"));
const organisationRequestAdmin_routes_1 = __importDefault(require("./organisationRequestAdmin.routes"));
const commission_routes_1 = __importDefault(require("./commission.routes"));
const withdrawal_routes_1 = __importDefault(require("./withdrawal.routes"));
const report_routes_1 = __importDefault(require("./report.routes"));
const router = (0, express_1.Router)();
router.use("/auth", auth_routes_1.default);
router.use("/students", student_routes_1.default);
router.use("/applications", application_routes_1.default);
router.use("/documents", document_routes_1.default);
router.use("/placements", placement_routes_1.default);
router.use("/payments", payment_routes_1.default);
router.use("/notifications", notification_routes_1.default);
router.use("/messaging", messaging_routes_1.default);
router.use("/support", support_routes_1.default);
router.use("/admin", admin_routes_1.default);
router.use("/agent", agent_routes_1.default);
router.use("/organizations", organization_routes_1.default);
router.use("/organisations", organisation_routes_1.default);
router.use("/organisation-requests", organisationRequest_routes_1.default);
router.use("/admin/organisations", organisationAdmin_routes_1.default);
router.use("/admin/organisation-requests", organisationRequestAdmin_routes_1.default);
router.use("/commissions", commission_routes_1.default);
router.use("/withdrawals", withdrawal_routes_1.default);
router.use("/reports", report_routes_1.default);
exports.default = router;
//# sourceMappingURL=index.js.map